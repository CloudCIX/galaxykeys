...
### Step 2: User Input ###
echo "[INFO] Remote Git Repository Backup Setup"

# Remote git backup (HTTPS with access token)
while true; do
    read -rp "Enter the remote HTTPS git repository URL (e.g., https://example.com/username/repo.git): " GIT_REPO_URL
    if [[ "$GIT_REPO_URL" =~ ^https://.+/.+\.git$ ]]; then
        echo "[WARNING] HTTPS URL detected. You'll need an access token for authentication."
        echo ""
        echo "To create a token (for GitLab, GitHub, or similar):"
        echo "  1. Go to your git hosting provider's user settings and find Access Tokens"
        echo "  2. Create a token with 'write_repository' and 'read_repository' scopes (or equivalent)"
        echo "  3. Copy the token (you won't see it again)"
        echo ""
        echo "[SECURITY NOTE] Token will be used temporarily and NOT saved anywhere."
        echo ""
        while true; do
            read -rp "Do you have an access token? (y/N): " has_token
            if [[ "$has_token" =~ ^[Yy]$ ]]; then
                echo ""
                echo "Enter your access token (input will be hidden):"
                read -rs GIT_TOKEN
                echo ""
                if [[ -n "$GIT_TOKEN" ]]; then
                    if [[ "$GIT_REPO_URL" =~ ^https://([^/]+)/(.+)$ ]]; then
                        ...
                        GIT_REPO_URL="https://oauth2:${GIT_TOKEN}@${GIT_HOST}/${GIT_PATH}"
...
- **Access is via HTTPS and access token only. SSH is not supported in this workflow.**
...
echo "[INFO] Adding remote git repository..."

# Remove existing origin if it exists
git remote remove origin 2>/dev/null || true

# Add the remote repository as origin
if ! git remote add origin "$GIT_REPO_URL"; then
    echo "[ERROR] Failed to add remote repository as origin." >&2
    exit 1
fi

echo "[OK] Remote repository added: $GIT_REPO_URL"

### Step 9: Test Connection and Push ###
echo "[INFO] Testing connection to remote repository..."
...
echo "[INFO] Pushing to remote git repository..."

# Push to the remote repository
if ! git push -u origin "$BRANCH_NAME"; then
    echo "[ERROR] Failed to push to remote repository." >&2
    echo "[INFO] You may need to:"
    echo "  1. Create the repository on your git hosting provider first"
...
echo "[OK] Successfully pushed to remote git repository!"
